// 🚀 game.js - ROYAL ESPORTS FULL 3D EDITION (ZERO LAG + HIGH QUALITY) 🚀
...
// ==========================================
// 🚀 INSTANCED MESH SYSTEM (ZERO LAG MAGIC) 🚀
// ==========================================
...
// 2. Static Models for Instancing (Tree, Rock, Grass)
    ...
    baseModel.traverse(c => {
        if(c.isMesh) {
            ...
            if(currentGraphics !== 'LOW') { 
                imesh.castShadow = (type !== 'grass'); // Grass ki shadow OFF taaki game speed makkhan rahe
...
window.toggleSpectate = function(e) { if(e) { e.preventDefault(); e.stopPropagation(); } isSpectating = !isSpectating; const btn = document.getElementById('spectate-btn'); if(isSpectating) { btn.innerText = "STOP SPECTATING"; btn.style.background = 'rgba(255, 0, 0, 0.6)'; btn.style.borderColor = '#ff4444'; } else { btn.innerText = "WATCH OPPONENT"; btn.style.background = 'rgba(0, 200, 255, 0.6)'; btn.style.borderColor = '#00ffff'; } }
...
window.buildMazeFromMap = function(mapArray, dynamicSize) { 
    ...
    while(window.mazeGroup.children.length > 0) window.mazeGroup.remove(window.mazeGroup.children[0]); walls = []; coinMeshes = {}; healthMeshes = {}; hasGameEnded = false; myCoins = 0; oppCoins = 0; myStatus = 'playing'; oppStatus = 'playing'; isSpectating = false; 
    ...
    // 🌳 JUNGLE ENVIRONMENT (150 Trees & Grass)
    ...
    for(let z = 0; z < size; z++) { 
        for(let x = 0; x < size; x++) { 
            ...
            if(mapArray[z][x] === 1) { 
                ...
                if(currentGraphics !== 'LOW') { oldW.castShadow = true; oldW.receiveShadow = true; } // WALL SHADOWS ON
                window.mazeGroup.add(oldW); 
                walls.push(new THREE.Box3().setFromObject(oldW));
            } 
            else { 
                let rand = Math.random();
                if(rand > 0.96) instData.rock.push({x: posX + (Math.random()-0.5)*2, y: 0, z: posZ + (Math.random()-0.5)*2, rot: Math.random()*Math.PI*2, scale: 0.8+Math.random()*0.5});
    ...
    let entryGate = window.createFortGate(); entryGate.position.set(entryPosX, 0, entryPosZ); window.mazeGroup.add(entryGate); let exitGate = window.createFortGate(); exitGate.position.set(exitPosX, 0, exitPosZ); window.mazeGroup.add(exitGate);
    ...
    if (appState === 'menu') { camera.position.lerp(new THREE.Vector3(0, 3.8, 8.0), 0.1); camera.lookAt(0, 1.5, 0); myPlayer.position.lerp(new THREE.Vector3(0, 0, 0), 0.1); if(!isDraggingLobby) lobbyRotationY += 0.005; myPlayer.rotation.y = lobbyRotationY; if(window.mazeGroup) window.mazeGroup.visible = false; opponentPlayer.visible = false; ghostGroup.visible = false; menuEnvGroup.visible = true; skyBox.position.copy(camera.position); } 
        ...
        if(isGameRunning) { 
            ...
            if (myStatus === 'playing') {
                ...
                for(let key in coinMeshes) { let c = coinMeshes[key]; if(myPlayer.position.distanceTo(c.position) < 2.0) { if(typeof playInstantSound === 'function') playInstantSound('coin'); myCoins++; document.getElementById('coin-counter').innerText = "COINS: " + myCoins; window.mazeGroup.remove(c); delete coinMeshes[key]; let coords = key.split('_'); window.gameState.map[coords[1]][coords[0]] = 0; if(!isSinglePlayer && window.sendEvent) window.sendEvent({ type: 'coin', x: coords[0], z: coords[1], score: myCoins }); } } 
...
window.removeCoinLocally = function(x, z, oppNewScore) { let key = `${x}_${z}`; if(coinMeshes[key]) { window.mazeGroup.remove(coinMeshes[key]); delete coinMeshes[key]; window.gameState.map[z][x] = 0; } oppCoins = oppNewScore; }; 
window.removeHealthKitLocally = function(key) { if(healthMeshes[key]) { window.mazeGroup.remove(healthMeshes[key]); delete healthMeshes[key]; } };
